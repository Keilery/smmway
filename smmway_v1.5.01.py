...
def _process_smm_order(state: dict, link: str) -> None:
    ...
    if CTX.storage.cfg.get("dynamic_workflows_enabled", True):
        ...
        if DW.should_soft_block(lot.service_id):
            logger.warning("DW: service %s is soft-blocked, notifying buyer", lot.service_id)
            send_buyer_message(chat_id, "⏳ Услуга временно перегружена. Заказ будет выполнен чуть позже.")
            # Ставим в очередь с задержкой (retry через 5 мин)
            threading.Timer(300, _process_smm_order, args=(state, link)).start()
            return
        # Anti-flood: задержка между заказами одной услуги
