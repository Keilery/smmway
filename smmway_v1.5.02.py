...
DEFAULT_CONFIG = {
    ...
    "auto_review_text": "Спасибо за отзыв! За ❤️ от вас — даём бонусную докрутку на тот же заказ.",
...
def _maybe_process_review(c: "Cardinal", msg) -> bool:
    ...
    # Авто-ответ на отзыв (если включено)
    _maybe_send_review(o)
    # Бонусная докрутка
...
def _format_order_started_notification(
    *,
    ...
    """Единое HTML-уведомление о запуске заказа в Telegram."""
    ...
    # Запуск заказа
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append("🚀 <b>Запуск заказа</b>")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append(f"👤 <b>Покупатель:</b> <code>{buyer}</code>")
    lines.append(
        f"🧾 <b>FunPay-заказ:</b> <a href=\"https://funpay.com/orders/{html_escape(funpay_order_id)}/\">"
        f"#{html_escape(funpay_order_id)}</a>"
    )
    lines.append(f"🛒 <b>Услуга:</b> {html_escape(short_service)} <i>(#{lot.service_id})</i>")
    lines.append(f"📦 <b>Лот FP:</b> <code>#{lot.funpay_lot_id}</code>")
    lines.append(f"🔢 <b>Объём:</b> <code>{quantity}</code> шт.")
    ...
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append(f"💵 На FunPay заплатили: <code>{funpay_price:.2f} ₽</code>")
    lines.append(f"💸 Нам на smmway:        <code>{smmway_charge_rub:.4f} ₽</code>")
    lines.append(
    ...
    lines.append("")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append("🤖 <b>Запуск бота</b>")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append(f"🆔 <b>SMMWay-заказ:</b> <code>#{smm_id}</code>")
    lines.append(f"🔗 <b>Цель:</b> <a href=\"{link_html}\">{link_label}</a>")

    # Баланс
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    lines.append("♻️<b>Балланс</b>")
    lines.append("━━━━━━━━━━━━━━━━━━━━")
    if balance_before is not None:
        lines.append(f"💰 SMMWay баланс до создания заказа: <code>{balance_before:.4f}</code> ₽")
    if balance_after is not None:
        lines.append(f"💰 SMMWay баланс после создания: <code>{balance_after:.4f}</code> ₽")
    ...
    try:
        ...
        if CTX.storage.cfg.get("notify_order_created"):
            ...
            try:
                ...
                pass
            # Единое уведомление (1 сообщение)
...
def _maybe_send_review(o: OrderEntry) -> None:
    """Отправляет наш ответ на отзыв покупателя (если включено и покупатель уже оставил отзыв)."""
    if not CTX.storage.cfg.get("auto_review_enabled"):
        return
    try:
        text = CTX.storage.cfg.get("auto_review_text") or ""
        CTX.cardinal.account.send_review(o.funpay_order_id, text)
    except Exception as ex:
        logger.debug("auto-review skipped for %s: %s", o.funpay_order_id, ex)
    ...
    def on_callback(c):
        try:
            ...
            if data == f"{CB}:autoreview":
                ...
                return
            if data == f"{CB}:autoreview_toggle":
                CTX.storage.cfg["auto_review_enabled"] = not CTX.storage.cfg.get("auto_review_enabled", True)
                CTX.storage.save_config()
                _show_autoreview_menu(c)
                return
            ...
            if data == f"{CB}:autoreview_stars_set":
                ...
                return
            if data == f"{CB}:autoreview_text_set":
                _ask_value(c, "auto_review_text",
                           "Введи текст нашего ответа на отзыв покупателя")
                return
            if data == f"{CB}:health":
                _show_health(c)
                return
...
def _show_autoreview_menu(c):
    ...
    K, B = _kbd()
    cfg = CTX.storage.cfg
    on_reply = cfg.get("auto_review_enabled", True)
    ...
    text_tmpl = cfg.get("auto_review_text", "")
    text = (
        "<b>⭐ Отзывы покупателей</b>\n\n"
        f"📝 Авто-ответ на отзыв: {'🟢 вкл' if on_reply else '🔴 выкл'}\n"
        f"   Текст: <code>{html_escape(text_tmpl)[:120]}</code>\n\n"
    ...
    kb = K(row_width=1)
    kb.add(B(("🔴 Выкл" if on_reply else "🟢 Вкл") + " авто-ответ на отзыв",
             callback_data=f"{CB}:autoreview_toggle"))
    kb.add(B("✏ Текст авто-ответа", callback_data=f"{CB}:autoreview_text_set"))
