...
DESCRIPTION = (
    ...
    "• Price Stabilizer — плавная цена, стабильные продажи без качелей.\n"
...
DEFAULT_CONFIG = {
    ...
    # --- Price Stabilizer: стабильные продажи без «качелей» цены ---
    # Вместо резких скачков (или полного пропуска обновления) цена двигается
    # к целевой плавно: мелкие колебания игнорируются (deadband), крупные —
    # ограничиваются шагом за цикл (max_step) и сглаживаются (EMA). Так цена и
    # позиция лота в выдаче FunPay держатся ровно, а продажи идут стабильно,
    # без всплесков-провалов из-за дёрганья цены вверх-вниз.
    "price_stabilizer_enabled": True,
    "price_deadband_pct": 3.0,       # не трогаем цену, если цель в пределах ±%
    "price_max_step_pct": 12.0,      # макс. изменение цены за один цикл, %
    "price_smoothing_factor": 0.5,   # EMA: доля шага к цели за цикл (0..1)
...
def _stabilize_price(current: float, target: float, cfg: dict) -> tuple[float, bool]:
    """Сглаживает движение цены лота к целевой ради стабильных продаж.

    Возвращает кортеж ``(новая_цена, нужно_ли_обновлять)``.

    Зачем: если просто пересчитывать цену «в лоб» каждый цикл, она прыгает
    вслед за курсом/ценой smmway. Скачки вверх роняют конверсию и позицию
    лота, скачки вниз бьют по марже — продажи идут рывками. Стабилизатор:
      * deadband — игнорирует мелкие колебания (цель близко к текущей цене);
      * EMA — двигает цену к цели плавно, долей ``smoothing_factor`` за цикл;
      * max_step — ограничивает изменение за один цикл, чтобы не было рывков.
    """
    if not current or current <= 0:
        return target, True
    deadband = float(cfg.get("price_deadband_pct", 3.0)) / 100.0
    max_step = float(cfg.get("price_max_step_pct", 12.0)) / 100.0
    alpha = float(cfg.get("price_smoothing_factor", 0.5))
    alpha = min(max(alpha, 0.0), 1.0)
    ratio = target / current
    # deadband — мелкие колебания не трогаем вовсе
    if abs(ratio - 1.0) <= deadband:
        return current, False
    # EMA-сглаживание: делаем лишь часть шага к цели
    smoothed = current + alpha * (target - current)
    # ограничиваем изменение за один цикл (clamp вместо резкого скачка)
    upper = current * (1.0 + max_step)
    lower = current * (1.0 - max_step)
    new_price = min(max(smoothed, lower), upper)
    new_price = round(new_price, 6)
    if new_price < 0.001:
        new_price = 0.001
    # Если после округления цена фактически не изменилась — не дёргаем FunPay
    if abs(new_price - current) < 1e-9:
        return current, False
...
def _maybe_process_review(c: "Cardinal", msg) -> bool:
    ...
    # Бонусная докрутка за положительный отзыв
...
def _maybe_send_review(o: OrderEntry) -> None:
    """Удалено в 1.5.03: авто-ответ на отзыв покупателя больше не отправляется.

    Функция оставлена как no-op для обратной совместимости (на случай, если
    где-то ещё остался вызов). Сам функционал «ответа на отзыв» убран.
    """
    return
...
def update_all_prices(force: bool = False) -> tuple[int, int]:
    ...
    stabilizer_on = bool(CTX.storage.cfg.get("price_stabilizer_enabled", True))
    for lot in list(CTX.storage.lots.values()):
        ...
        target_price = compute_fp_price(s, markup_pct=markup, rate=rate, min_price=min_price)
        if stabilizer_on and lot.last_price_fp and not force:
            # Плавно двигаем цену к цели (deadband / EMA / max_step) — ради
            # ровных продаж без «качелей». force=True (ручной пересчёт) идёт
            # мимо стабилизатора, чтобы сразу применить точную цену.
            new_price, need_update = _stabilize_price(lot.last_price_fp, target_price, CTX.storage.cfg)
            if not need_update:
                continue
        else:
    ...
    def on_callback(c):
        try:
            ...
            if data == f"{CB}:autoprice_interval":
                ...
                return
            if data == f"{CB}:autoprice_stab_toggle":
                CTX.storage.cfg["price_stabilizer_enabled"] = not CTX.storage.cfg.get("price_stabilizer_enabled", True)
                CTX.storage.save_config()
                _show_autoprice_menu(c)
                return
            if data == f"{CB}:autoprice_deadband":
                _ask_value(c, "price_deadband_pct",
                           "Мёртвая зона в % — цену не трогаем, если цель в этих пределах "
                           "(напр. <code>3</code>):",
                           cast=float, after_cb=f"{CB}:autoprice")
                return
            if data == f"{CB}:autoprice_maxstep":
                _ask_value(c, "price_max_step_pct",
                           "Макс. изменение цены за один цикл, % (напр. <code>12</code>):",
                           cast=float, after_cb=f"{CB}:autoprice")
                return
            if data == f"{CB}:autoprice_smooth":
                _ask_value(c, "price_smoothing_factor",
                           "Коэффициент сглаживания EMA от 0 до 1 — доля шага к цели за цикл "
                           "(напр. <code>0.5</code>):",
                           cast=float, after_cb=f"{CB}:autoprice")
                return
            if data == f"{CB}:notif":
            ...
            if data == f"{CB}:update_apply":
                ...
                if result:
                    ...
                    if ok:
                        bot.answer_callback_query(c.id, f"Обновлено до {new_ver}!")
                    else:
                        bot.answer_callback_query(c.id, "Ошибка обновления")
                else:
...
def _show_autoprice_menu(c):
    ...
    stab = cfg.get("price_stabilizer_enabled", True)
    text = (
        f"<b>⭐ Авто-цена</b>\n"
        f"Вкл: {'🟢' if on else '🔴'}\n"
        ...
        f"<b>📈 Стабилизатор цены: {'🟢 вкл' if stab else '🔴 выкл'}</b>\n"
        f"   Мёртвая зона: <b>{cfg.get('price_deadband_pct', 3.0)}%</b>\n"
        f"   Макс. шаг/цикл: <b>{cfg.get('price_max_step_pct', 12.0)}%</b>\n"
        f"   Сглаживание (EMA): <b>{cfg.get('price_smoothing_factor', 0.5)}</b>\n"
        "   <i>Двигает цену к цели плавно — продажи идут ровно, без качелей.</i>"
    )
    kb = K(row_width=2)
    ...
    kb.add(B(("🔴 Выкл" if stab else "🟢 Вкл") + " стабилизатор",
             callback_data=f"{CB}:autoprice_stab_toggle"))
    kb.add(B("🎯 Мёртвая зона %", callback_data=f"{CB}:autoprice_deadband"),
           B("📏 Макс. шаг %", callback_data=f"{CB}:autoprice_maxstep"))
    kb.add(B("〰 Сглаживание", callback_data=f"{CB}:autoprice_smooth"))
    kb.add(B("◀️ Меню", callback_data=f"{CB}:main"))
    bot.edit_message_text(text, c.message.chat.id, c.message.id, reply_markup=kb, parse_mode="HTML")
    bot.answer_callback_query(c.id)
...
def _show_autoreview_menu(c):
    ...
    text = (
        ...
        "<i>Авто-ответ на отзыв отключён в этой версии.</i>"
    )
    kb = K(row_width=1)
...
def _show_update_menu(c):
    ...
    text = (
        f"<b>\ud83d\udce5 \u041e\u0431\u043d\u043e\u0432\u043b\u0435\u043d\u0438\u044f SMMWay</b>\n\n"
        f"\u0422\u0435\u043a\u0443\u0449\u0430\u044f \u0432\u0435\u0440\u0441\u0438\u044f: <code>{VERSION}</code>\n"
        f"{status_text}\n\n"
        f"\u0410\u0432\u0442\u043e-\u043e\u0431\u043d\u043e\u0432\u043b\u0435\u043d\u0438\u0435: {'🟢 \u0412\u043a\u043b' if enabled else '🔴 \u0412\u044b\u043a\u043b'}\n"
        f"\u0418\u043d\u0442\u0435\u0440\u0432\u0430\u043b: {CTX.storage.cfg.get('auto_update_interval_sec', 3600)} \u0441\u0435\u043a"
    )
    kb = K(row_width=1)
    kb.add(B(("🟢" if enabled else "🔴") + " \u0410\u0432\u0442\u043e-\u043e\u0431\u043d\u043e\u0432\u043b\u0435\u043d\u0438\u0435",
             callback_data=f"{CB}:update_toggle"))
    kb.add(B("\ud83d\udd0d \u041f\u0440\u043e\u0432\u0435\u0440\u0438\u0442\u044c \u043e\u0431\u043d\u043e\u0432\u043b\u0435\u043d\u0438\u044f",
